...
function Profile(props) {
  ...
  const saveEditHandler = (toSave) => {
    ...
    props.editProfile(props.userData._id, props.token, userProfile);
  };
  const newPostHandler = () => {
    setStoryModal(
      <StoryModal
        type="newPost"
        card={{ title: "", body: "", privacy: false }}
      />
    );
      ...
      {props.loadingUserData || props.loadingUserStory ? (
        ...
        <>
          <div>
            <div className="introHeader">
              {!profileDetails["coverQuoteDisplay"] ? (
                ...
                <ProfileEditModel
                  onChange={(e) => inputChangeHandler(e, "coverQuote")}
            ...
            <div className="sub-header">
              <div className="sub-items">
                <h2 className="name">{props.userData.userName}</h2>
                {!profileDetails["bioDisplay"] ? (
                  ...
                  <ProfileEditModel
                    onChange={(e) => inputChangeHandler(e, "bio")}
          ...
          <div className="profile">
            {userId === props.loggedInUserId ? (
              <div className="new-post" onClick={newPostHandler}>
                <input
                  type="text"
...
const mapDispatchToProps = (dispatch) => {
  return {
    fetchUserProfile: (userId, token) =>
      dispatch(actions.fetchUserProfile(userId, token)),
    editProfile: (userId, token, useProfile) =>
      dispatch(actions.editProfile(userId, token, useProfile)),
